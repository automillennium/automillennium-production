...
export default function LanguageDropdown({ currentLang }) {
  ...
  const changeLanguage = (newLang) => {
    ...
    // ✅ Add zh check
    if (["en", "ar", "zh"].includes(segments[0])) {
      segments[0] = newLang;
    } else {
      segments.unshift(newLang);
    }
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            style={{
              transformOrigin:
                currentLang === "ar" ? "top left" : "top right",
            }}
          >
            <div className="flex flex-col">
              {languages.map((langItem) => (
                ...
                >
                  <span
                    className={clsx({
                      "font-arabic": langItem.code === "ar",
                      "font-chinese": langItem.code === "zh", // ✅ Optional if you want a custom Chinese font
                    })}
                  >
                    {langItem.name}
                  </span>
