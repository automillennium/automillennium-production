...
//                 className="px-8 py-3 bg-blue-600 rounded-lg hover:bg-blue-700 tr ansition-colors font-medium"
//               >
//                 Retry Loading
//               </button>
//             </div>
...
const Page = ({ params }) => {
  ...
  // OPTIMIZATION: Keep essential loading only for dictionary/essential data
  const [isLoadingEssential, setIsLoadingEssential] = useState(true);
  const [videoError, setVideoError] = useState(false);
  // REMOVED: [isVideoLoaded, setIsVideoLoaded] - not needed for a static video background
  ...
  // Combined effect for essential loading state
  useEffect(() => {
    if (isDictionaryLoaded) {
      // Content can display as soon as the dictionary is loaded
  ...
  // ----------------------------------------------------
  // VIDEO HANDLERS - MODIFIED (Simplified)
  // ----------------------------------------------------
  ...
  const handleVideoError = () => {
    console.error("Main video failed to load. Falling back.");
    setVideoError(true);
    // Treat as "loaded" for content display since we removed the video loader
    ScrollTrigger.refresh(true);
  };
  ...
  // ----------------------------------------------------
  // INITIAL VIDEO CHECK - REMOVED: Video ready logic
  // ----------------------------------------------------
  useEffect(() => {
    const video = videoRef.current;
    if (video) {
      // We only care about errors now; the video should attempt to play immediately.
      video.addEventListener("error", handleVideoError);
    }
  ...
  }, []); // Empty dependency array as the video logic is now simplified
  
  // ----------------------------------------------------
  // GSAP ANIMATIONS (Unchanged)
  // ----------------------------------------------------
  useGSAP(() => {
    // GSAP logic will run as soon as isDictionaryLoaded is true
  ...
  }, [isDictionaryLoaded]); // Dependent on isDictionaryLoaded

  // ----------------------------------------------------
  // LOADER - Now only blocks for essential data (dictionary)
  // ----------------------------------------------------
  ...
  // ----------------------------------------------------
  // DYNAMIC CONTENT DATA (Unchanged)
  // ----------------------------------------------------
  const services = [
    {
      title: dictionary?.vtxPage?.services?.ppf?.title || "Paint Protection Film (PPF)",
      imagePlaceholder: "bg-cyan-500/20",
      videoUrl: "/videos/PPF.mp4",
      posterUrl: "/img/PPF-poster.jpg",
      items: dictionary?.vtxPage?.services?.ppf?.items || [
        "Invisible shield against chips, scratches, and debris.",
        "High gloss finish & Self-healing technology.",
        "Custom-cut for precision fit.",
      ],
    ...
    {
      title: dictionary?.vtxPage?.services?.ceramic?.title || "Ceramic Coating",
      imagePlaceholder: "bg-blue-500/20",
      videoUrl: "/videos/CERAMIC_.mp4",
      posterUrl: "/img/CERAMIC_-poster.jpg",
      items: dictionary?.vtxPage?.services?.ceramic?.items || [
        "Long-lasting hydrophobic protective layer.",
        "Maximum UV resistance & deep gloss shine.",
        "Easy maintenance with anti-dirt properties.",
      ],
    ...
    {
      title: dictionary?.vtxPage?.services?.tinting?.title || "Window Tinting",
      imagePlaceholder: "bg-indigo-500/20",
      videoUrl: "/videos/TINT.mp4",
      posterUrl: "/img/TINT-poster.jpg",
      items: dictionary?.vtxPage?.services?.tinting?.items || [
        "Superior heat rejection for cabin comfort.",
        "Full UV protection for occupants.",
        "Stylish, privacy-enhancing, and legally compliant shades.",
      ],
    ...
    {
      title: dictionary?.vtxPage?.services?.detailing?.title || "Interior & Exterior Detailing",
      imagePlaceholder: "bg-violet-500/20",
      videoUrl: "/videos/TINT.mp4",
      posterUrl: "/img/DETAILING-poster.jpg",
      items: dictionary?.vtxPage?.services?.detailing?.items || [
        "Deep cleaning and material restoration.",
        "Leather, fabric, and dashboard long-term protection.",
        "Keeps your vehicle fresh inside-out.",
      ],
    ...
    {
      title: dictionary?.vtxPage?.services?.custom?.title || "Tailored Protection (Custom Packages)",
      imagePlaceholder: "bg-purple-500/20",
      videoUrl: "/videos/TINT.mp4",
      posterUrl: "/img/CUSTOM-poster.jpg",
      items: dictionary?.vtxPage?.services?.custom?.items || [
        "Protection packages customized for your vehicle's needs.",
        "Combine PPF, coating, tinting & detailing for total coverage.",
        "Expert consultation to choose the optimal package.",
      ],
  ...
  return (
    <div className="bg-black" ref={pageContainer} dir={lang === "ar" ? "rtl" : "ltr"}>
      <div id="about" className="min-h-screen w-screen bg-black">
        ...
        <div className="relative w-full h-screen">
          ...
          <video
            ...
            muted
            playsInline
            // preload="auto" is a good default, keep it
            preload="auto" 
            // poster is crucial to show content instantly
            poster="/img/vtx-fallback-poster.jpg" 
            onError={handleVideoError}
            // Removed isVideoLoaded from the className as the video should be visible immediately
            className={`absolute top-0 left-0 w-full h-full object-cover transition-opacity duration-1000`}
          >
            <source
              src="https://automillennium.com/wp-content/uploads/2023/08/VTX.mp4"
              ...
              />
            <p className="text-white absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2">
              {dictionary?.globalBusiness?.browserSupport || "Your browser doesn't support HTML5 video."}
            </p>
          </video>
        ...
        {/* Main Content - Now always visible after dictionary load */}
        <div className={`relative opacity-100`}> 
          ...
          <div className="relative flex flex-col items-center pt-40 pb-20 px-4 md:px-8">
            ...
            <img
              src="/img/vtx-white.png"
              alt={dictionary?.vtxPage?.logoAlt || "VTX Logo"}
              className="page-logo-img h-[150px] w-[200px] lg:h-[300px] lg:w-[400px] object-contain" 
            />



            {/* Title and Intro Text - Unchanged */}
            <div className="text-center max-w-5xl mx-auto mt-10">
              <h1 className="page-intro-text text-white text-4xl md:text-6xl font-extrabold tracking-tight mb-6 leading-tight">
                {dictionary?.vtxPage?.title || "Premium Protection. Advanced Technology."}
              </h1>
              <p className="page-intro-text text-gray-300 text-xl md:text-2xl max-w-4xl mx-auto">
                {dictionary?.vtxPage?.description || 
                  "VTX is Auto Millennium's premium line of vehicle protection solutions, designed to keep your car looking brand-new while safeguarding it from harsh weather, road debris, and everyday wear. Our products combine advanced technology with professional application, ensuring your vehicle maintains its value, shine, and performance over time."}
              </p>
            </div>
          </div>
          ...
          <section className="py-24 px-4 md:px-6 max-w-[90%] mx-auto">
            <h2 className="fade-scroll text-white text-center text-5xl md:text-[56px] leading-[1.0714285714] font-semibold tracking-[-0.005em] font-sans mb-16">
              {dictionary?.vtxPage?.servicesTitle || "Our Protection Services"}
            </h2>
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-6 mt-10 justify-center">
              {services.map((service, i) => (
                <div key={i} className="flex flex-col group w-full min-w-[150px] 2xl:max-w-[300px] mx-auto"> 
                  ...
                  {/* Service Details Card - Unchanged */}
                  <div className="service-item p-4 bg-zinc-900 border border-zinc-800 rounded-b-3xl shadow-xl transition duration-300 group-hover:shadow-cyan-500/20 group-hover:bg-zinc-800/80 min-h-[190px] 2xl:min-h-[240px]">
                    <h3 className="text-[12px] 2xl:text-xl font-semibold text-white mb-3 fade-scroll ">
                      {service.title}
                    </h3>
                    
                    <ul className="list-disc text-gray-400 pl-5 text-[10px] 2xl:text-sm space-y-1"> 
                      {service.items.map((item, j) => (
                        <li key={j} className="fade-scroll">{item}</li>
                      ))}
                    </ul>
                  </div>
                </div>
            ...
            <div className="text-center text-white max-w-md mx-4 p-8 rounded-xl bg-zinc-800 border border-zinc-700">
              <div className="mb-6">
                ...
                <h3 className="text-xl font-semibold mb-2">
                  {dictionary?.globalBusiness?.videoErrorTitle || "Video Failed to Load"}
                </h3>
                <p className="text-gray-300">
                  {dictionary?.globalBusiness?.videoErrorText || "There was a problem loading the video content. Displaying fallback image."}
                </p>
              </div>
              <button
                onClick={retryVideoLoad}
